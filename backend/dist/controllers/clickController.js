...
exports.getClickStats = exports.getClickHistory = exports.recordClick = void 0;
...
const recordClick = async (req, res) => {
    try {
        ...
        const click = new Click_1.default({
            userId,
            coordinates,
            ipAddress,
            userAgent,
        ...
        if (lastClickDate) {
            ...
            const isNewDay = now.getDate() !== lastClickDate.getDate() ||
            ...
            const isYesterday = yesterday.getDate() === lastClickDate.getDate() &&
...
const getClickHistory = async (req, res) => {
    try {
        ...
        const clicks = await Click_1.default.find({ userId })
            .sort({ timestamp: -1 })
            .skip(skip)
            .limit(limit);
        const total = await Click_1.default.countDocuments({ userId });
        res.status(200).json({
            clicks,
            pagination: {
                total,
...
const getClickStats = async (req, res) => {
    try {
        ...
        const hourlyClicks = await Click_1.default.aggregate([
            {
                $match: {
        ...
        const dailyClicks = await Click_1.default.aggregate([
            {
                $match: {
                    userId: new mongoose_1.default.Types.ObjectId(userId.toString()),
